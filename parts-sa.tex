...
and an example is depicted in \pref{fig:sa-livelock}.
...
starting from a local state of the same automaton that is not in the initial context,
but that may be active at some point of the solving.
Second, \pref{def:lcg}\nobreakdash-\ref{lcg-conn},
allows to re-target objectives whose own solving changes their target local state.
This happens when playing the actions that activate the local states required to solve
an objective $P$ also changes the active local state of $\PHsort(P)$.
