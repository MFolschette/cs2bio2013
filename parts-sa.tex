...
\modmf{%
The previous subsections permitted to define all the tools needed to build the Local Causality Graph (LCG), such as the on depicted in \pref{fig:sa-livelock}.
...
linking them to objectives (\eg, $\PHobj{w}{u}$, if $w \in \ctx$),
and locally solving these objectives in order to create new required local states
from other automata.
}
...
where the bounce local state of an objective may be changed.
First,
...
starting from another local state of the same automaton that may be active at some point.
Second, \pref{def:lcg}\nobreakdash-\ref{lcg-conn},
allows to re-target objectives whose solving changes their target local state.
This happens if the actions allowing to activate the local processes required to solve
an objective $P$ change the active process of $\PHsort(P)$.
In this case, the objective changes to another objective $\PHobj{w}{\bounce{P}}$.
...
and did not require this kind of synchronization.
Nevertheless, \pref{def:lcg} is equivalent to the definition of LCG
given in~\cite{FPMR13-CS2Bio},
although adapted for AANs which notably allowed to simplify its formalism.
}
