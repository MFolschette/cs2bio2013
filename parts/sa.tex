...
that respects all Criteria (\ref{cr:bounded}, \ref{cr:cyclefreeness} \& \ref{cr:compcs}) of \pref{ssec:hypothesis}.
...
The reachability property studied so far concern a single process, \ie{} the
...
The sufficient condition for the reachability of a process from a given context
can be extended for the successive reachability of several processes ($a_i$,
then $b_j$, etc.) in the following manner.
...
where $\ctx' = \procs(\mycwB{\ctx}{P})\Cap\lastprocs(P)$
\end{theorem}
