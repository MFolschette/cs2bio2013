...
The graph of local causality $\cwB = (\V, \E)$ defined in \pref{def:glc} is a graph where $\V \subset \Proc \cup \Obj \cup \Sol$ and $\E \subset \V \times \V$.
...
An objective $P \in \Obj$ is solvable if the abstractions of bounce sequences $\aBS(P) \in \Sol$ (\pref{def:aBS}) can be reached (\pref{eq:ESol1}), leading to several required processes (\pref{eq:ESol2}).
If $a$ is a component, the reachability of a process $a_i$ is approximated by the ability to solve all objectives $\PHobjp{a}{j}{i} \in \Obj$ for all $a_j$ in the initial context (\pref{eq:EReq});
if $a$ is a cooperative sort, the reachability of $a_i$ is directly solved by the set of the processes $\csState(a_i)$ (\pref{def:csState}) it represents (\pref{eq:EPrio}).
Finally, if the resolution of an objective $P$ requires a process of $\PHsort(P)$, \ie $\gCont(\PHsort(P), P) \neq \emptyset$ (\pref{def:maxCont}), then this objective is re-targeted (\pref{eq:ECont}).
