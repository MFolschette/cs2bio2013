...
and for all $ps \in \powerset(\Proc) \cup \PHl$, $ps \subseteq \ctx \EQDEF \forall a_i \in ps, a_i \in \ctx$.
...
\begin{definition}
  ...
  where $\myB = (\cwV, \cwE)$ is the smallest graph where: $\cwV = \VProc \cup \VObj \cup \VSol$ with $\VProc \subset \PHproc$, $\VObj \subset \Obj$, $\VSol \subset \powerset(\PHproc)$ and $\cwE \subset \cwV \times \cwV$ so that:
  \begin{align}
...
\label{def:coherent}
  ...
  it does not belong to $(\PHproc \times \powerset(\PHproc))$
  or if it does and $y$ has no process successor $a_j \in \Bv \cap \PHproc$ so that $\exists a_i \in y$, $a_i \neq a_k$.
...
\begin{itemize}
  \item If $(P, \emptyset) \in \Bsol$, either $\PHtarget(P) = \PHbounce(P)$ and $\delta = \emptyseq$;
    ...
    and $\delta = \delta_1 \PHplay \zeta_1 \PHplay \dots \PHplay \delta_{|\zeta|} \PHplay \zeta_{|\zeta|}$ is a sequence given by \pref{th:hcompcomp}.
