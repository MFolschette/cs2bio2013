...
% * On applique les règles suivants de parcours où, à tout moment, s représente l'état courant, et l'état du parcours peut être « en descente » ou « en montée » :
% 
% - Dans un nœud a_k \in LS :
%   - Si en cours de descente, descendre dans le nœud s[a] />* a_k \in Obj.
%   - Si en cours de remontée, monter dans le nœud Sol parent et enlever l'élément a_k de son marquage.
% 
% - Dans un nœud a_j />* a_k \in Obj :
%   - Si en cours de descente, descendre dans un nœud pps \in NSol au choix. Marquer ce nœud avec une séquence \zeta \in BS tq \zeta^ = pps.
%   - Si en cours de remontée, monter dans le nœud LS ou Obj parent.
% 
% - Dans un nœud pps \in NSol (marqué par \zeta) :
%   - Si \zeta ≠ \emptyseq, descendre dans le nœud ps \in Sol tq ps = hitters(\zeta_1). Marquer le nœud ps par l'ensemble ps.
%   - Si \zeta = \emptyseq, alors remonter dans le nœud Obj parent et continuer la remontée.
% 
% - Dans un nœud Sol marqué par m :
%   - Si m ≠ \emptyset, alors descendre dans un des nœuds fils a_k \in LS tq a_k \in m.
%   - Si m = \emptyset, alors remonter dans le nœud NSol parent, marqué par \zeta. Si \zeta_1 est jouable, jouer cette action et marquer le nœud par \zeta_{2..|\zeta|}. Si \zeta_1 n'est pas jouable (c.-à-d. si target(\zeta_1) a changé), aller au nœud s[sort(bounce(\zeta_1))] />* bounce(\zeta_1) \in Obj et commencer une descente.
...
%%%%%%%% Tentative 2 %%%%%%%%%%%%%%%%%%%%%%%
% % \newcommand{\resP}{\mathbf{\Obj res}}
% % \newcommand{\resOS}{\mathbf{\OS res}}
% \newcommand{\resP}{\mathsf{res}^\Obj}
% \newcommand{\resOS}{\mathsf{res}^\OS}
% 
% \newcommand{\res}{\mathbf{res}}
% \newcommand{\autoBS}{\mathbf{autoBS}}
% \newcommand{\rec}{\mathbf{rec}}
% 
% Pour tout objectif $P$ dans $\cwB$,
% on note $X_P$ une séquence d'objectifs obtenue en effectuant
% un parcours en profondeur d'abord de $\cwB$.
% Un tel parcours existe car $\cwB$ ne comporte pas de cycle.
% 
% $$\res_s(\w) = \resOS_s((X_{\w_i})_{i \in \indexes{\w}})$$
% 
% \[
%   \resOS_s(\w) \DEF
%     \begin{cases}
...
%         & \text{ if $\w = \emptyseq$} \\
%       \resP_s(\w_1) \concat
%       \resOS_{s \play \resP_s(\w_1)}(\w_{2..\card{\w}})
%         & \text{ otherwise} \\
%     \end{cases}
% \]
% 
% $
% \resP_s(\PHobj{a_j}{a_k}) \DEF
%   \todo{resP}$
% 
% % $\resP_s(P) \DEF
% %     \begin{cases}
