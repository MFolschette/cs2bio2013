...
%          If $\PHget{s \PHplay \delta_1}{a} = a_j$, then from \pref{th:vplay}, $\exists \delta'_1 \in \Sce$, $\zeta_1$ can be played in $s \PHplay \delta_1 \PHplay \delta'_1$.
        \item[*] For $n \in \indexes{\zeta}$, let $s_n = s \PHplay \delta_1 \PHplay \delta'_1 \PHplay \zeta_1 \PHplay \dots \PHplay \delta_{n-1} \PHplay \delta'_{n-1} \PHplay \zeta_{n-1}$ (or $s_1 = s$),
          and $\zeta_n = \PHhit{b_i}{a_j}{a_k}$. By hypothesis, $\exists \delta_n \in \muconcr_{s_n}(\PHobj{\any}{b_i})$, and we have: $\PHget{s_n \PHplay \delta_n}{a} \in \bounceprocs_{s_n} \subset \bounceprocs_\ctx$ \todo{À montrer}.
          ...
          If $\PHget{s_n \PHplay \delta_n}{a} = a_j$, then from \pref{th:vplay}, $\exists \delta'_n \in \restriction{\Sce}{k-1}$, $\zeta_n$ can be played in $s \PHplay \delta_n \PHplay \delta'_n$
          \todo{$\delta'_n$ ne perturbe pas $b$ ?}
          .
