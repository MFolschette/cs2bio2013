...
and $\PHh'^{(2)} = \PHh^{(2)} \cup \{ \PHfrappe{\pfp_s(\total)}{\reach_0}{\reach_1} \}$;
therefore, the additional action in $\PHh'^{(2)}$ allows to conclude on the reachability of process $\pfp_s(\total)$, that is, on the reachability of $s$ (considering only the components).
