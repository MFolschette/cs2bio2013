...
We also extend the notion of override of a context by a set of processes (\pref{def:ctxcap}).
...
Finally, bounce sequences on a sort (\pref{def:bs}) are sequences of actions in which the bounce of each action equals the hitter of the following action.
...
Bounce sequences can be abstracted into the sets of hitters of every action that are different from the considered sort (\pref{def:aBS}).
...
The set of all processes involved in such a graph $(\V, \E)$ is given in \pref{def:allprocs}.
...
by computing $\gCont_\ctx(\PHsort(P), P)$.
\begin{definition}
\label{def:allprocs}
  \begin{align*}
...
\towrite{expliquer le graphe de causalité}
...
\begin{example}
  ...
  Indeed, the under-approximation given in \pref{th:approxinf} does not conclude as one of the edges of the resulting graph of local causality is not coherent (\pref{def:coherent}).
