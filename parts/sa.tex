...
%    so that: $\PHget{(s \PHplay \delta_n \PHplay \delta')}{\PHsort(p_{n+1})} = p_{n+1}$.
    ...
    Finally, by theorem \todo{théorème mise à jour}, there exists a scenario $\delta'' \in \restriction{\Sce}{1}$
    that can be played in $s \PHplay \delta_n \PHplay \delta'$
    and so that, if we denote $\delta_{n+1} = \delta_n \PHplay \delta' \PHplay \delta''$,
    we have: $\update(s \PHplay \delta_n \PHplay \delta') = s \PHplay \delta_{n+1}$ (\todo{théorème stabilité}), and the same property about processes.
\end{itemize}
...
%      We build recursively a scenario $\delta$. Let $m = |\indexes{\zeta}|$.
      \begin{itemize}
        ...
        By \todo{théorème mise à jour} there exists $\delta'' \in \restriction{\Sce}{1}$ so that $\update(s \PHplay \delta') = s \PHplay \delta' \PHplay \delta''$.
        Furthermore, $\PHget{(s \PHplay \delta' \PHplay \delta'')}{b} = b_j$ \todo{car $b \in \components$ ou $b$ est stable}.
