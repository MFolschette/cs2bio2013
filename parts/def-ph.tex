...
in which actions of $\PHh^{(1)}$ are called \emph{prioritised actions} and allow to model non-biological actions.
...
\begin{example}
  The PH in \pref{fig:ph-livelock} contains three components ($a$, $b$ and $c$) and a cooperative sort ($ab$, which models cooperation between sorts $a$ and $b$).
\end{example}
