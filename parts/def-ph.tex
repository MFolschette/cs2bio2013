...
A PH with $k$ priorities (\pref{def:ph}), also simply called PH \towrite{abréviation ?} in the following, gathers a finite number of concurrent \emph{processes} divided into a finite set of \emph{sorts}.
...
\end{definition}
\begin{definition}[Dynamics of a Process Hitting with $k$ priorities]
\label{def:play}
...
In \pref{def:restriction}, we define the $n$-reduction of a given PH as the PH with $n$ priorities in which only actions of priority lower or equal to $n$ of the original PH are kept.
\begin{definition}[Process Hitting $n$-reduction] %\towrite{Revoir terminologie : “reduction” ? “restriction” ?}
\label{def:restriction}
  If $\PH = (\PHs; \PHl; \PHa^{\langle k \rangle})$ is a Process Hitting with $k$ priorities and $n \in \segm{1}{k}$, we denote $\restriction{\PH}{n}$ %\towrite{$\PH_{|n}$ $\PH'$}
...
\begin{example}
  \pref{fig:ph-livelock} gives an example of PH with $2$ priorities, with $\PHs = \{ a, b, c, ab \}$ and:
  \begin{align*}
    \PHl_a &= \{ a_0, a_1 \} \enspace, & \PHl_b &= \{ b_0, b_1 \} \enspace, \\
    \PHl_c &= \{ c_0, c_1 \} \enspace, & \PHl_{ab} &= \{ ab_{00}, ab_{01}, ab_{10}, ab_{11} \} \enspace.
  \end{align*}
  Actions of priority $1$ are represented with double arrows and actions of priority $2$ are represented with single arrows.
  The hit part of each action in drawn in plain line and the bounce part is in dotted line.
  The greyed processes stand for a possible state $s = \PHstate{a_1, b_0, c_0, ab_{10}}$.
...
This allows to gain the same expressivity in PH than in boolean networks, as explained in \towrite{section 2 ou 3}.
...
\pref{cr:bounded} states that the dynamics of the studied model $\PH$ contains no infinite sequence of uncontrollable actions.
...
\pref{def:chainsactions} defines chains of uncontrollable actions,
and \pref{cr:cyclefreeness} prevents the presence of cycles in these chains to avoid incoherent states between cooperative sorts.
\begin{definition}
...
In \pref{def:neighbours} we define the notion of neighbour \towrite{revoir terminologie : neighbors ? vicinity ?} sorts and actions of a sort.
...
A well-formed cooperative sort allows to define a cooperation between sorts as presented in \pref{ssec:cooperation};
...
Furthermore, because of (\ref{css}), we denote $\csState(a_i)$ the set of sub-states represented by the process $a_i$ of any cooperative sort $a$ (\pref{def:csState}).
...
In the following we will simply write “component” (resp.~“cooperative sort”) instead of “well-formed component” (resp.~“well-formed cooperative sort”).
...
\label{cr:compcs}
\towrite{Problème : sortes non frappées dans $\components$ ou $\cs$ ?}
...
\todo{The criteria given in this subsection allow to define a class of PH models that share the same behaviour than discrete models.
The demonstration of this statement is found in Annex B.}
...
\begin{proof}
  ...
  As there is no cycle of actions between the cooperative sorts (\pref{cr:cyclefreeness}) and an updated cooperative sort cannot evolve, at most $|\cs|$ updates can be done.
\end{proof}
\pref{th:hcompcomp} states that for a given state $s$ and for any action $h = \PHhit{a_i}{b_j}{b_k}$ between components,
...
\end{theorem}
\begin{proof}
  Similar to the proof of \pref{th:hcompcomp}, and
  %From \pref{th:update}, there exists such a scenario $\delta$ so that: $(s \PHplay \delta) = \update(s)$.
