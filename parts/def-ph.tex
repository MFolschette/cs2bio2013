...
\begin{definition}[Well-formed component]
\label{def:component}
  ...
  We denote $\components\subset\Sigma$ the set of well-formed components.
\end{definition}
...
These definitions are permitted by \pref{cr:cyclefreeness}.
...
\label{def:csState}
  ...
  $$\csState(a_i) = \{ ps \in \PHsubl[\PHl]_{\Vs(a)} \mid \forall s \in \PHl, \pfp_{s \Cap ps}(a) = a_i \}$$
...
A translation of ADN into PH and its correction are given in \pref{sec:dn}.
...
\label{def:update}
  ...
  \end{align*}
\todo{Extend to $s \in \PHsubl[\PHl]$ to use it in $\PHflat$?}
\end{definition}
%
\begin{theorem}
\label{th:update}
