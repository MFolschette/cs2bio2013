...
\section{Weak Bisimulation of Asynchronous Discrete Networks (\pref{ssec:hypothesis})}
...
Let us denote by $\toPH\DNdef$ the encoding of the ADN $\DNdef$ in Process Hitting with $2$ classes
of priorities (\pref{def:DN2PH}).
...
%$f^i_{\varsigma'}$ where $\get{\varsigma'}{a^j}=a^j_k$.
