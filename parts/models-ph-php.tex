...
In this section, we give a translation that allows to obtain,
from any PH model with any number of priorities,
a PH model with 2 classes of priorities
that respects all Conditions of \pref{ssec:hypothesis}.
Such a translation is thus particularly useful to be able to study the dynamics of
any kind of PH model by using the static analysis developed in \pref{sec:sa},
which is not possible if these conditions are not met.

We consider in the following a PH with $k \in \sNN$ classes of priories
...
\begin{itemize}
  ...
  \item if the $i$\textsuperscript{th} conjunction of $\Fop{h}$ consists of only one element $p$,
    then \modMF{this conjunction} can be translated as a regular action $\PHhit{p}{\target{h}}{\bounce{h}}$ instead of a cooperative sort
