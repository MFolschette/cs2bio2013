...
\todo{Prove that $\PHflat(\oPH)$ respects all Conditions!}
...
With \pref{th:ppplaysubset}, we can then characterise the playability of an action in a state only with a sub-state.
...
\ie for $i \in \segm{1}{\n}$.
This construction allows to obtain the same dynamics as $\oPH$, which is proven by \pref{th:bisimPHP}.
%
\begin{theorem}
...
with $2$ classes of priorities with the same dynamics that respects all Conditions of \pref{ssec:hypothesis}.
