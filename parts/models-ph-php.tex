...
and to adapt the behaviour of the model to make any action unplayable
...
\begin{definition}[Playability property language ($\F$)]
  ...
  \end{itemize}
  If $P \in \F$ is a playability property, we denote by: $\PHsortf{P}$
  the set of sorts whom at least one local state is mentioned in $P$.
  Then, if $\mysigma \in \PHsubl$
  with $\PHsortf{P} \subseteq \PHsortf{\mysigma}$,
...
and every other action of higher priority has to be not playable.
...
\begin{definition}[Flattening ($\PHflat$)]
  ...
  If $k \in \sNN$, $k > 1$ and $\oPH = (\ov{\PHs}; \ov{\PHl}; \ov{\PHa}^{\angles{k}})$ is an AAN$k$,
  we denote by
